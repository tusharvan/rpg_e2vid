...
def make_event_preview(events, mode="red-blue", num_bins_to_show=-1):
    ...
    if num_bins_to_show < 0:
        ...
        sum_events = (
            torch.sum(events[0, -num_bins_to_show:, :, :], dim=0).detach().cpu().numpy()
        )
        ...
        if options.hot_pixels_file:
            try:
                self.hot_pixel_locations = np.loadtxt(
                    options.hot_pixels_file, delimiter=","
                ).astype(np.int)
                print(
                    "Will remove {} hot pixels".format(
                        self.hot_pixel_locations.shape[0]
                    )
                )
            except IOError:
                print(
                    "WARNING: could not load hot pixels file: {}".format(
    ...
    def __init__(self, options):
        ...
        if self.output_folder:
            ...
            if self.save_events:
                ...
                print(
                    "Will write event previews to: {}".format(
    ...
    def __call__(self, img, event_tensor_id, stamp=None, events=None):
        ...
        if self.save_events and events is not None:
            ...
            cv2.imwrite(
                join(
                    self.event_previews_folder,
                    "events_{:010d}.png".format(event_tensor_id),
                ),
                event_preview,
            )
        ...
        cv2.imwrite(
            join(
                self.output_folder,
                self.dataset_name,
                "frame_{:010d}.png".format(event_tensor_id),
            ),
            img,
        )
...
class UnsharpMaskFilter:
    ...
    def __init__(self, options, device):
        ...
        self.gaussian_kernel = (
            gkern(self.gaussian_kernel_size, self.unsharp_mask_sigma)
            .unsqueeze(0)
            .unsqueeze(0)
            .to(device)
        )
    ...
    def __call__(self, img):
        if self.unsharp_mask_amount > 0:
            with CudaTimer("Unsharp mask"):
                blurred = F.conv2d(
                    img, self.gaussian_kernel, padding=self.gaussian_kernel_size // 2
                )
                img = (
                    1 + self.unsharp_mask_amount
...
class ImageFilter:
    ...
    def __call__(self, img):
        ...
        if self.bilateral_filter_sigma:
            with Timer(
                "Bilateral filter (sigma={:.2f})".format(self.bilateral_filter_sigma)
            ):
