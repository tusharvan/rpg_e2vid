...
from .utils.inference_utils import (
    upsample_color_image,
    merge_channels_into_color_image,
...
class ImageReconstructor:
    ...
    def initialize(self, height, width, options):
        ...
        if self.perform_color_reconstruction:
            self.crop_halfres = CropParameters(
                int(width / 2), int(height / 2), self.model.num_encoders
            )
                ...
                for channel in events_for_each_channel.keys():
                    ...
                    with CudaTimer("Tensor (GPU) -> NumPy (CPU)"):
                        reconstructions_for_each_channel[channel] = (
                            new_predicted_frame[
                                0, 0, crop.iy0 : crop.iy1, crop.ix0 : crop.ix1
                            ]
                            .cpu()
                            .numpy()
                        )
